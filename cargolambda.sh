...
project_name='cargolambda'
command_path="${this_directory}/cli/commands"

code_workspace="cargolambda.code-workspace"

_help() {
  echo "Command List"
  echo "    edit|code    : Open in VSCode"
  echo "    example      : Run example command"
  echo "    devcontainer : Dev Container commands"
  echo ""
}
...
_cargolambda() {
  ...
  case "$command" in
    ...
    devcontainer ) script_name="$command_path/devcontainer.sh" ;;
    *) _help; exit 0 ;;
  esac
